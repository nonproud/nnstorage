...
if($row){
	...
	session_write_close();
	printf('<script> alert("Welcome %s %s!"); window.location.replace("home.php");</script>',$_SESSION['fname'],$_SESSION['lname']);
	echo $date;
}else{
	echo '<script> alert("Something went wrong!"); window.location.replace("home.php"); </script>';
}
$conn->close();
?>